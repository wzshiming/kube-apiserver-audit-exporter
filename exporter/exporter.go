...
func (p *Exporter) updateMetrics(event auditv1.Event) {
	...
	if event.ObjectRef != nil {
		...
		case "pods":
			if event.ObjectRef.Subresource == "binding" {
				if event.Level == auditv1.LevelMetadata &&
					event.Stage == auditv1.StageResponseComplete &&
					event.ResponseStatus.Code == http.StatusCreated {
					p.recordPodBinding(event)
				}
			} else {
				if event.Level == auditv1.LevelRequestResponse &&
					event.Stage == auditv1.StageResponseComplete &&
					event.ResponseStatus.Code == http.StatusCreated {
					p.recordPodCreateAndDelete(event)
				}
			}
		...
		case "jobs":
			if event.Level == auditv1.LevelRequestResponse &&
				event.Stage == auditv1.StageResponseComplete &&
				event.ResponseStatus.Code == http.StatusOK {
				p.recordBatchJobComplete(event)
			}
		}
	}
}
...
func (p *Exporter) recordBatchJobComplete(event auditv1.Event) {
	var job BatchJob
...
func (p *Exporter) recordPodBinding(event auditv1.Event) {
...
func (p *Exporter) recordPodCreateAndDelete(event auditv1.Event) {
	switch event.Verb {
	case "delete":
